...
<div class="page-title-container">
    <div class="container">
        <div class="page-title pull-left">
            <h2 class="entry-title">Pacote - Editando "{{$evento->nome_br}}"</h2>
        </div>
    </div>
</div>
@stop
...
{{Form::model($evento, array('url' => array("admin/eventoespecial", $evento->id), 'method' => 'PUT', 'files' => true))}}
		...
		<div class="block">
		    ...
		    <div class="tab-container trans-style box">
		        ...
		        <div class="tab-pane fade" id="hoteis" data-ng-controller="buscaController as buscaCtrl" data-ng-init="buscaCtrl.objectListSelected = {{htmlspecialchars($evento->hoteis)}}; buscaCtrl.model = 'hotel';">
		                ...
		                <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($apartamentos as $ap)
                                    <tr>
                                        <td> <input type="checkbox" name="apartamentos[]" value="{{$ap->id}}" @forelse($evento->apartamentos as $pa) @if($ap->id == $pa->id) checked="checked" @endif @empty @endforelse /> </td>
                                        <td>{{$ap->nome_br}}</td>
                                        <td>{{$ap->nome_en}}</td>
                                        <td>{{$ap->pais->name}}</td>
                                    </tr>
		                ...
		                <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($passeios as $pa)
                                    <tr>
                                        <td> <input type="checkbox" name="passeios[]" value="{{$pa->id}}" @forelse($evento->passeios as $pa2) @if($pa->id == $pa2->id) checked="checked" @endif @empty @endforelse /> </td>
                                        <td>{{$pa->nome_br}}</td>
                                        <td>{{$pa->nome_en}}</td>
                                        <td>{{$pa->pais->name}}</td>
                                    </tr>
		                ...
		                <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($servicosnoturnos as $sn)
                                    <tr>
                                        <td> <input type="checkbox" name="servicosnoturnos[]" value="{{$sn->id}}" @forelse($evento->servicosnoturnos as $sn2) @if($sn->id == $sn2->id) checked="checked" @endif @empty @endforelse /> </td>
                                        <td>{{$sn->nome_br}}</td>
                                        <td>{{$sn->nome_en}}</td>
                                        <td>{{$sn->pais->name}}</td>
                                    </tr>
