...
{{Form::model($hotel, array('url' => array("admin/hotel", $hotel->id), 'method' => 'PUT', 'files' => true))}}
<!-- <form method="POST" action="{{URL::to('admin/hotel/')}}" enctype="multipart/form-data"> -->
	...
	<div class="block">
	    <img id="img" src="@if($hotel->imagem) uploads/hoteis/{{$hotel->imagem}} @else uploads/hoteis/no-img.png @endif" alt="" width="570" height="300" style="width: 100%; cursor: pointer;">
	                ...
	                <div class="tab-pane" id="hotel-amenities">
                       	...
                       	@foreach($caracteristicas as $c)	                        
	                            <li class="col-md-4 col-sm-6">
	                                <div class="icon-box style1"><i class="{{$c->icone}}"></i><input type="checkbox" name="caracteristicas[]" value="{{$c->id}}" @if(count($hotel->caracteristicas) > 0) @foreach($hotel->caracteristicas as $hc) @if($hc->id == $c->id) checked="checked" @endif  @endforeach  @endif/> {{$c->nome_br}}</div>
	                            </li>	                        
                        @endforeach   
                        </ul>    
...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		...
		@if($hotel->imagens->count() > 1)
			var i = {{$hotel->imagens->count()}}
		@else
			var i = 1;
		@endif
