...
<div class="page-title-container">
    <div class="container">
        <div class="page-title pull-left">
            <h2 class="entry-title">Pacote - Editando "{{$pacote->nome_br}}"</h2>
        </div>
    </div>
</div>
@stop
...
{{Form::model($pacote, array('url' => array("admin/pacote", $pacote->id), 'method' => 'PUT', 'files' => true))}}
		...
		<div class="block">
			<img id="img" src="@if($pacote->imagem) uploads/pacotes/{{$pacote->imagem}} @else images/no-img.png @endif" alt="" width="570" height="300" style="width: 100%; cursor: pointer;">
		    <div class="tab-container trans-style box">
		        ...
		        <div class="tab-pane fade" id="hoteis" data-ng-controller="buscaController as buscaCtrl" data-ng-init="buscaCtrl.objectListSelected = {{htmlspecialchars($pacote->hoteis)}}">
		                ...
		                <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($passeios as $pa)
                                    <tr>
                                        <td> <input type="checkbox" name="passeios[]" value="{{$pa->id}}" @forelse($pacote->passeios as $pa2) @if($pa->id == $pa2->id) checked="checked" @endif @empty @endforelse /> </td>
                                        <td>{{$pa->nome_br}}</td>
                                        <td>{{$pa->nome_en}}</td>
                                        <td>{{$pa->pais->name}}</td>
                                    </tr>
		                ...
		                <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($servicosnoturnos as $sn)
                                    <tr>
                                        <td> <input type="checkbox" name="servicosnoturnos[]" value="{{$sn->id}}" @forelse($pacote->servicosnoturnos as $sn2) @if($sn->id == $sn2->id) checked="checked" @endif @empty @endforelse /> </td>
                                        <td>{{$sn->nome_br}}</td>
                                        <td>{{$sn->nome_en}}</td>
                                        <td>{{$sn->pais->name}}</td>
                                    </tr>
...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		...
		@if($pacote->imagens->count() > 1)
			var i = {{$pacote->imagens->count()}}
		@else
			var i = 1;
		@endif
