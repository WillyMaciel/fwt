...
<div class="page-title-container">
    <div class="container">
        <div class="page-title pull-left">
            <h2 class="entry-title">{{trans('menu.passeios')}}</h2>
        </div>
    </div>
</div>
@stop
                ...
                <div class="toggle-container filters-container">
                    ...
                    <div class="panel style1 arrow-right">
                        ...
                        <div id="modify-search-panel" class="panel-collapse collapse">
                            <div class="panel-content">
                                <form method="get" action="{{URL::to('passeio/')}}">
                    ...
                    @foreach($passeios as $passeio)
                        <article class="box">
                            <figure class="col-sm-5 col-md-4">
                                <a title="" href="{{URL::to("passeio/show/{$passeio->id}")}}" class="hover-effect"><img width="270" height="160" alt="" src="@if(isset($passeio->imagem)) uploads/passeios/270x160_{{$passeio->imagem}} @else http://placehold.it/270x160 @endif"></a>
                            </figure>
                            <div class="details col-sm-7 col-md-8">
                                <div>
                                    <div>
                                        <h4 class="box-title"><a href="{{URL::to("passeio/show/{$passeio->id}")}}"> @if(Session::get('lang') == 'pt') {{$passeio->nome_br}} @else {{$passeio->nome_en}} @endif </a><small><i class="soap-icon-departure yellow-color"></i> {{$passeio->pais->name}} - {{$passeio->estado}} </small></h4>

                                    </div>
                                    <div>
                                        <div title="" class="five-stars-container" data-toggle="tooltip" data-placement="bottom" data-original-title="{{$passeio->estrelas or 0}} Estrelas">
                                                <span class="five-stars" style="width: {{$passeio->estrelas * 20}}%;"></span>
                                        </div>
                                        <span class="review">{{$passeio->estrelas or 0}} {{trans('hotel.estrelas')}}</span>
                                    </div>
                                </div>
                                <div>
                                    <p>@if(Session::get('lang') == 'pt') {{{strip_tags(substr($passeio->descricao_br, 0, 150)) . ' ...'}}} @else {{{strip_tags(substr($passeio->descricao_en, 0, 150)) . ' ...'}}} @endif</p>
                                    <div>
                                        <span class="price"><small>{{trans('carrinho.preco')}}</small>{{$passeio->valor or '--'}}</span>
                                        <a class="button btn-small full-width text-center" title="" href="{{URL::to("passeio/show/{$passeio->id}")}}">{{trans('hotel.selecionar')}}</a>
                                    </div>
                                </div>
                            </div>
                        </article>
