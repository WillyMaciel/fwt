                ...
                <div class="tab-content">
                    <div id="photos-tab" class="tab-pane fade in active">
                        <div class="photo-gallery style1" data-animation="slide" data-sync="#photos-tab .image-carousel">
                            <ul class="slides">
                                <li><img src="@if($translado->imagem) uploads/translados/900x500_{{$translado->imagem}} @else http://placehold.it/900x500}} @endif" alt="" /></li>
...
We suggest that you contact the host to confirm availability and rates before submitting a reservation request.
                ...
                <div class="tab-content">
                    <div class="tab-pane fade in active" id="hotel-description">
                        ...
                        <div class="long-description">
                            <h2>@if(Session::get('lang') == 'pt') Sobre {{$translado->nome_br}} @else About {{$translado->nome_en}} @endif</h2>
                            <p>@if(Session::get('lang') == 'pt') {{$translado->descricao_br}} @else {{$translado->descricao_en}} @endif</p>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="hotel-reviews">
                        @if(!$translado->reviews->isEmpty())
                        <div class="guest-reviews">
                            <h2>Guest Reviews</h2>
                            @foreach($translado->reviews as $review)
                    ...
                    <div class="tab-pane fade" id="hotel-write-review">
                        @include('forms.review', array('action' => URL::to('review'), 'produto_id' => $translado->id))
                    </div>
                </div>
        ...
        <div class="sidebar col-md-3">
            <article class="detailed-logo">
                <figure>
                    <img width="114" height="85" src=" @if($translado->imagem) uploads/translados/114x85_{{$translado->imagem}} @else http://placehold.it/114x85 @endif" alt="">
                </figure>
                <div class="details">
                    <h2 class="box-title">@if(Session::get('lang') == 'pt') {{$translado->nome_br}} @else {{$translado->nome_en}} @endif<small><i class="soap-icon-departure yellow-color"></i><span class="fourty-space">@if(Session::get('lang') == 'pt') {{$translado->pais->name}} @else {{$translado->pais->name}} @endif</span></small></h2>
                    <span class="price clearfix">
                        <small class="pull-left">{{trans('carrinho.preco')}}</small>
                        <span class="pull-right">{{$translado->valor or '--'}}</span>
                    </span>
                    <div class="feedback clearfix">
                        <div title="" class="five-stars-container" data-toggle="tooltip" data-placement="bottom" data-original-title="{{$translado->estrelas or 0}} {{trans('hotel.estrelas')}}">
                            <span class="five-stars" style="width: {{$translado->estrelas * 20}}%;"></span>
                        </div>
                        <span class="review pull-right">{{$translado->estrelas or 0}} {{trans('hotel.estrelas')}}</span>
                    </div>
                    <p class="description">@if(Session::get('lang') == 'pt') {{substr($translado->descricao_br, 0, 150) . ' ...'}} @else {{substr($translado->descricao_en, 0, 150) . ' ...'}} @endif</p>
                        <!-- CARRINHO -->
                        <form action="{{URL::to('carrinho/add')}}" method="POST">
                            <input type="hidden" name="produto" value="{{$translado->id}}" />
